...
describe('arXiv search tests', () => {
	...
	it('should return results as expected - default values', async () => {
		...
		expect(mockAxiosGet).toHaveBeenCalledWith(
			'https://export.arxiv.org/api/query?search_query=all:RNN+AND+all:Deep learning+ANDNOT+all:LSTM+OR+all:GAN&start=0&max_results=10'
		);
	...
	it('should return results as expected', async () => {
		...
		expect(mockAxiosGet).toHaveBeenCalledWith(
			'https://export.arxiv.org/api/query?search_query=all:RNN+AND+all:Deep learning+ANDNOT+all:LSTM+OR+cat:GAN&start=10&max_results=50'
		);
		expect(results).toMatchSnapshot();
	});
	it('should return results as expected - with sortBy and sortOrder', async () => {
		...
		expect(mockAxiosGet).toHaveBeenCalledWith(
			'https://export.arxiv.org/api/query?search_query=all:RNN+AND+all:Deep learning+ANDNOT+all:LSTM+OR+all:GAN&start=10&max_results=50&sortBy=relevance&sortOrder=ascending'
		);
		expect(results).toMatchSnapshot();
	});
